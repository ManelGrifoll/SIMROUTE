...
'''   setled=np.zeros(shape=(Ny*Nx,4))
       ...
       Route optimized (without waves):CostConst,distConst         
       Route optimized (with waves):  CostWave,distWave
       Route Minimum Distance with waves: CostCtw,distConst
 ''' 
 ##
''' 
     Fem primer 
...
frep.write(st)
print(st)
st='Route optimized (without waves):         {:6.2f}            {:6.2f}    \n'
print(st.format(CostConst,distConst))
frep.write(st.format(CostConst,distConst))
st='Route optimized (with waves):            {:6.2f}            {:6.2f}    \n'
frep.write(st.format(CostWave,distWave))
print(st.format(CostWave,distWave))
st='Route Minimum Distance with waves:       {:6.2f}            {:6.2f}    \n'
print(st.format(CostCtw,distConst))
frep.write(st.format(CostCtw,distConst))
st='\n========================================================================\n\n'
...
np.savez_compressed('out/'+name_Simu,prs,hs,fp,dir,L_Trip,L_TripFix,
                    L_CostTrip,L_ConsCostTrip,L_CostFix,ARX)
######################
frep.write(st)
frep.close()
...
report='out/Rout_'+name_Simu+'.txt'
frout=open(report,'w')
st='  nnod     Lon    Lat      Cost    hm         \n'
frout.write(st)
tXr=1234
ll=1.234
latlat=-3.235
cc=100.456
hhh=11.23
st='{:7d} {:8.3f} {:7.3f} {:7.3f} {:5.2f}\n'.format(tXr,ll,latlat,cc,hhh)
frout.write(st)
frout.close()
