...
for t in infotos:
    ...
    if t <L_CostTrip[-1]:
        tl=distL(L_CostTrip,t)   
    else:
        tl=len(L_CostTrip)
    if t <L_CostFix[-1]:
        tc=distL(L_CostFix,t)  
    else:
        tc=len(L_CostFix)
    ...
    ax.set_title(strtim)
    if t !=0:
        ax.plot(lonc,latc,'orange',transform=pc,label='Minimum distance route') 
    ...
    ax1.plot([nodes[nodIni,0]],[nodes[nodIni,1]],'^b',transform=pc,label='Departure')
    ax1.plot([nodes[nodEnd,0]],[nodes[nodEnd,1]],'*r',transform=pc,label='Arrival') 
