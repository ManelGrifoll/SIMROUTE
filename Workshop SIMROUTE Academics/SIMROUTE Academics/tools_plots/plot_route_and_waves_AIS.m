...
display(['The time of AIS route is ' num2str(route(end,1)) ' hours']);
%AIS_vs_OPT=(route(end,1)-q.cost_opt)
Saved_Percentage=(1-(q.cost_opt/route(end,1)))*100;
display(['Time saved following optimum route versus real route ' num2str(Saved_Percentage) '%']);
return
...
plot(q.nodes(q.nods_trip(pos_ok),1),q.nodes(q.nods_trip(pos_ok),2),'*g')
plot(q.nodes(q.nods_trip(1:pos_ok),1),q.nodes(q.nods_trip(1:pos_ok),2),'g','linewidth',2)
