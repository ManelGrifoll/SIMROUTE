...
Versio -01-2022  # afegida funcio testVrtx
Versió 29-01-2021
...
# si tenim el gel el posem, si no, no el carreguem                        
if os.path.exists(arx_ice):                        
    if t_ini==0:
        dat2=np.load(arx_ice)
        af=dat2['arr_0']
        th=dat2['arr_1']
        ...
        af=np.delete(af1,tt,axis=1)
        del af1
        th=np.delete(th1,tt,axis=1)
        del th1
...
def veloc(v0,nod_i,nod_f,cost_i):
   ...
   if time_res==1: 
       ...
       SICm=0.5*(af[nod_i,iv]+af[nod_f,iv])
       SITm=50*(th[nod_i,iv]+th[nod_f,iv])
...
#       print("veloc  ", hm   )
 ...
 #      hm=0.5*(hm1+hm2)
   ...
   if WEN_form==1:
       ...
       vel=min(v0-reduc_v_bow(angEnc)*hm*hm*3.2808*3.2808,reduc_v_ice(SICm,SITm))#transform meters to feets
   elif WEN_form==5:
       vel=min(v0-reduc_v_arte(angEnc,hm,Lbp,v0),reduc_v_ice(SICm,SITm))
   elif WEN_form==6:
       vel=min(v0-reduc_v_khok(angEnc,hm)*(1 - 1.35e-6*DWT*v0),reduc_v_ice(SICm,SITm))
   elif WEN_form==7:
       vel=reduc_v_ice(SICm, SITm)
#
   else:
       vel=v0
   
   if vel<=0:
       print('It is not possible to complete the trip with the current wave and ice conditions. Use other Wave Effect on Navigation formulation or shceddule for another time.') 
       print(vel,angEnc,ang_ship,nod_i,nod_f,hm,SICm,SITm,cost_i )
       raise SystemExit
...
def reduc_v_ice(SICm,SITm):
    #maximum speed the vessel can sail in given ice conditions
    if Vessel_type == 'OW':
        if SITm <= 0:
            IM = 2;
            
        if (SITm > 0 and SITm < 15):
            IM = 1;
            
        if (SITm >= 15 and SITm < 70):
            IM = -1;
            
        if (SITm >= 70 and SITm < 120):
            IM = -2;
        
        if (SITm >= 120 and SITm < 250):
            IM = -3;
    ...
    if Vessel_type == 'PC6':    
        ...
        if (SITm >= 250 and SITm < 300):
            IM = -3;
            
        if SITm >= 300:
            IM = -4
        
    ice_numeral = IM*10*SICm + 20*(1-SICm);
    
    if ice_numeral < 0:
        ice_speed = 0.1;
    if (ice_numeral >= 0 and ice_numeral <8):
        ice_speed = 4;
    if (ice_numeral >= 8 and ice_numeral <13):
        ice_speed = 5;
    if (ice_numeral >= 13 and ice_numeral <15):
        ice_speed = 6;
    if (ice_numeral >= 15 and ice_numeral <16):
        ice_speed = 7;
    if (ice_numeral >= 16 and ice_numeral <17):
        ice_speed = 8;
    if (ice_numeral >= 17 and ice_numeral <18):
        ice_speed = 9;
    if (ice_numeral >= 18 and ice_numeral <19):
        ice_speed = v0-np.ceil((v0-9)*2/3);
    if (ice_numeral >= 19 and ice_numeral <20):
        ice_speed = v0-np.ceil((v0-9)*1/3);
    if ice_numeral >= 20:
        ice_speed = v0;
